...
RegisterServerEvent('sy_carkeys:BuyKeys', function(plate, model)
    ...
    if ox_inventory:CanCarryItem(source, Keys.ItemName, 1) then
        if xPlayer.getMoney() >= Keys.CopyPrice then
            ...
            TriggerClientEvent('sy_carkeys:Notification', xPlayer.source, locale('title'), locale('NoDinero'), 'error')
        end
    end
end)
        ...
        MySQL.Async.execute(
            'UPDATE `owned_vehicles` SET `plate` = @newPlate, `vehicle` = @newVehicle WHERE `owner` = @identifier AND `plate` = @oldPlate',
            {
                ...
                if rowsChanged > 0 then
                    TriggerClientEvent('sy_carkeys:SetMatricula', xPlayer.source, newPlate, newColor)
                    TriggerClientEvent('sy_carkeys:Notification', xPlayer.source, locale('MatriculaActualizada',oldPlate, newPlate))
                else
